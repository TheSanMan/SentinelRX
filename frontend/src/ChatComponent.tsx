...
export default function ChatComponent({ meds }: { meds: Medication[] }) {
  ...
  return <section className="feature-card assistant-view" aria-labelledby="assistant-heading"><div className="assistant-heading"><div><span className="overline">SENTINEL · DRUGBANK</span><h2 id="assistant-heading">Ask Sentinel</h2><p>A local open-source model answers from DrugBank records and your saved list.</p></div><span className="context-pill"><Icon name="capsule" size={16}/>{meds.length} in context</span></div>
    <div className="conversation" role="log" aria-label="Conversation">{messages.length === 0 ? <div className="assistant-intro"><div className="assistant-icon"><Icon name="conversation" size={30}/></div><h3>What would you like to know?</h3><p>Ask Sentinel about a drug, your list, recorded interactions, or food warnings.</p><div className="prompt-list">{suggestions.map(question => <button key={question} onClick={() => { setInput(question); inputRef.current?.focus(); }}>{question}<Icon name="arrow" size={18}/></button>)}</div></div> : messages.map((message, index) => message.content && <article key={index} className={`chat-message ${message.role} ${message.error ? "error" : ""}`}><span>{message.role === "user" ? "YOU" : "SENTINEL"}</span><div className="markdown"><ReactMarkdown>{message.content}</ReactMarkdown></div></article>)}{busy && <div className="thinking" role="status"><Icon name="spinner" size={18} className="spin"/>Sentinel is reviewing DrugBank records…</div>}<div ref={end}/></div>
