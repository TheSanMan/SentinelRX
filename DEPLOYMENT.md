...
6. Run `docker compose up --build -d`. Check `https://sentinelrx.VM_PUBLIC_IP.sslip.io/api/health`; it should report `{"status":"ok"}`. Caddy obtains and renews a public HTTPS certificate once DNS and ports are correct. Test search, an interaction check, a label photo, and an assistant answer on the public address before sharing it.
