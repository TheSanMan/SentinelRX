...
3. Allow inbound TCP 80 and 443 in both the Oracle network security rules and the VM firewall. Register `sentinelrx` at [DuckDNS](https://www.duckdns.org/) and set its IPv4 address to the VM's public IP. The existing fallback hostname is `sentinelrx.VM_PUBLIC_IP.sslip.io` after replacing `VM_PUBLIC_IP` with that IP. [sslip.io](https://sslip.io/) resolves the embedded IP without an account. Update the DuckDNS record if the VM's public IP changes.
...
5. Copy `deploy/Caddyfile.example` to `deploy/Caddyfile` and replace `VM_PUBLIC_IP` with the actual IP. It serves both the DuckDNS name and the fallback hostname. If deploying a separate copy, replace `sentinelrx.duckdns.org` with your own DNS name or remove it. This configuration is public; add authentication if the deployment should be private. The live Caddyfile is ignored by Git.
6. Run `docker compose up --build -d`, then `docker compose exec ollama ollama pull qwen2.5:1.5b-instruct` to install Sentinel's Apache-2.0-licensed local model. The model download is about 1 GB and persists in the `ollama_models` volume. The API falls back to database summaries if the model is unavailable or already answering another request.
7. Check `https://sentinelrx.duckdns.org/api/health`; it should report `{"status":"ok"}`. Caddy obtains and renews a public HTTPS certificate once DNS and ports are correct. Test search, an interaction check, a label photo, and an assistant answer on the public address before sharing it.

The included Compose configuration exposes only Caddy to the internet. The database stays a read-only local mount. Each device stores its own medication list in browser local storage. The two hostnames have separate browser storage, so a list saved under the fallback address does not appear automatically under DuckDNS. Clearing Safari website data removes the list. No cross-device sync or accounts are included.
